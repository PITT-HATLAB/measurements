...
def sample_first_joystick(Params, stepsizes, cycle_Buttons = [1, 2], act_Param = 0, change_Axes= ['left_trigger','right_trigger'], change_Buttons = [3,4], stepsize_change_buttons = [13, 16], debug_Button = 9):
    """
    Grab 1st available gamepad, logging changes to the screen.
    L & R analogue triggers set the vibration motor speed.
    """
    [stepsizedown, stepsizeup] = stepsize_change_buttons
    [ax_down, ax_up] = change_Axes
    quit_Button = 14
    debug = 0

    print(f"Initial_Index: {act_Param}")
    
    def Bump(Param, step): 
        Param(Param()+step)
        print(f'{Param.name} += {step}\n{Param.name} = {Param()}')
    ...
    def on_button(button, pressed):
        ...
        nonlocal act_Param
        if pressed ==1: 
            if button == stepsizeup: 
                stepsizes[act_Param] *=10
                print(f'{Params[act_Param].name} stepsize = {stepsizes[act_Param]}')
            if button == stepsizedown: 
                stepsizes[act_Param] /=10
                print(f'{Params[act_Param].name} stepsize = {stepsizes[act_Param]}')
            if button == cycle_Buttons[0]:
                act_Param = (act_Param+1)%np.size(Params)
                print(f'Active Parameter: {Params[act_Param]}\nValue: {Params[act_Param]()}\nStepsize = {stepsizes[act_Param]}')
            if button == cycle_Buttons[1]:
                act_Param = (act_Param-1)%np.size(Params)
                print(f'Active Parameter: {Params[act_Param]}\nValue: {Params[act_Param]()}\nStepsize = {stepsizes[act_Param]}')
            if button == change_Buttons[0]:
                Bump(Params[act_Param], -stepsizes[act_Param])
            if button == change_Buttons[1]:
                Bump(Params[act_Param], +stepsizes[act_Param])
            if button == debug_Button:
                debug = int(not debug)
                print(f"Debug mode {bool(debug)}")
            if button ==quit_Button: 
                raise(Exception("Quitting Controller"))
    ...
    def on_axis(axis, value):
        ...
        right_speed = 0
        if np.abs(value) != 0: 
            if axis == change_Axes[0]:
                left_speed = value*1.5
                try: 
                    Bump(Params[act_Param], -stepsizes[act_Param]*value)
                except: 
                    print("non-int input for int variable, use change_buttons instead")
                
            elif axis == change_Axes[1]:
                right_speed = value
                try: 
                    Bump(Params[act_Param], stepsizes[act_Param]*value)
                except: 
...
'''
cycle_Button = 10, change_Axes= 
['left_trigger','right_trigger'], stepsize_change_buttons = [13, 16], Parameters = ['Param1', 'Param2'], ['stepsize1', 'stepsize2']
'''