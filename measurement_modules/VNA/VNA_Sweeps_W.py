...
def find_peak(frequencies, trace):
    ...
    peaks, props = find_peaks(trace, 8, distance=dist, prominence=5, width=width)
...
class Gain_Power_vs_Flux(): 
    ...
    '''
    def __init__(self, CS_settings, Gen_settings, VNA_settings, cwd, filename): 
        
        [self.CS,c_start,c_stop,c_steps] = CS_settings
        self.currents = np.linspace(c_start, c_stop, c_steps)
        [self.Gen] = Gen_settings
        [self.VNA, self.VNA_avg_number] = VNA_settings
        self.name = filename
        self.datadir = cwd
    ...
    def save_saturation_datapoint(self, current, gen_freq, gen_power, vna_freq, vna_input_powers, gains):
        ...
        self.sat_writer.add_data(
            ...
            sat_gen_power = [gen_power],
            sat_vna_freq = [vna_freq],
            sat_vna_powers = vna_input_powers.reshape(1,-1),
    ...
    def saturation_sweep(self, bias_current, gen_freq, gen_power, vna_freq, vna_p_start, vna_p_stop, vna_num_pts, vna_p_avgs): 
        ...
        prev_sweep_type = self.VNA.sweep_type()
    ...
    def sweep_power_for_gain(self, stepsize = 0.01, limit = 10, target_gain = 20, threshold = 2, saturation_sweep = True, vna_p_start = -43, vna_p_stop = 0, vna_p_steps = 1000, vna_p_avgs = 100): 
        ...
        pow_arr = np.arange(starting_power-5*stepsize, starting_power+5*stepsize, stepsize)
        #cut it off at 20dB and -20dB (generator range)
                #(-1)^n
        alt_array = np.empty(11)
        alt_array[0] = 0
        alt_array[1::2] = -1
        alt_array[2::2] = 1
        step_array = np.linspace(0,10, 11)
        alt_step_array = step_array*alt_array
        
        big_pow_step = 10*stepsize
    ...
    def sweep_gain_vs_freq(self, gen_freqs, cwd, filename, stepsize = 0.01, limit = 10, target_gain = 20, threshold = 2, saturation_sweep = True, vna_p_start = -43, vna_p_stop = 0, vna_p_steps = 1000, vna_p_avgs = 100):
        '''
        Parameters
        ----------
        VNA: Instrument
        ...
        for freq in gen_freqs: 
            print(f"Adjusting frequency: {freq}")
            self.Gen.frequency(freq)
            self.Gen.power(prev_pow)
            pow_arr, gain_arr, [closest_power, closest_val] =  self.sweep_power_for_gain(stepsize = stepsize, limit = limit, target_gain = target_gain, threshold = threshold)
